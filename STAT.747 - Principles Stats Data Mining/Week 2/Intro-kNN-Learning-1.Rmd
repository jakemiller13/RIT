---
title: "Introduction to Nearest Neighbors Learning"
...
The famous Pima Indian Diabetes dataset is well known benchmark data used by my
Machine Learning researchers in the context of classification learning. The input
space in this case is only 7-dimensional and the classification is binary, but the
underlying classification turns out to be rather challenging
...
Now let's a table of the data top of the data
```{R}
library(xtable)
xtable(head(xy))
```
...
## How about pairwise relationships
```{R}
